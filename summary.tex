...
\begin{itemize}
	...
	\item $\partial \lVert \mathbf{X} \lVert_2 =\lVert \mathbf{X}^T \lVert_2$
...
\begin{itemize}
    \item \textbf{Gaussian}: $ \N(x | \mu, \sigma^2) = \frac{1}{\sqrt{2 \pi \sigma^2}} \exp(- \frac{(x - \mu)^2}{2 \sigma^2})$
...
\begin{itemize}
    ...
    \begin{myalign*}
        \*w = \argmin_{\* w} \sum^N \frac{1}{2\sigma^2}(\*y_n - \*x^T \*w)^2 + \frac{\lambda}{2} \lVert\*w \lVert^2
    \end{myalign*}   
\end{itemize}
...
\begin{itemize}
      ...
      \end{myalign*}
      (i) there is a relationship between $\eta$ and $\mu$ throught the \textbf{link function}
      \begin{myalign*}
