...
\begin{itemize}
	...
	\end{myalign*}
	\begin{myalign*}
	    p(y_n = \*C_1| \*x_n) = \sigma(\tilde{\*x}^T \bm w)\\
	    p(y_n = \*C_2 | \*x_n) = 1 - \sigma(\tilde{\*x}^T \bm w)
	\end{myalign*}
	\item The probabilistic model:
	\begin{myalign*}
	    p(\*y | \*X, \bm w) = \prod_{n = 1}^N \sigma(\tilde{\*x}_n^T \bm w)^{y_n}(1 - \sigma(\tilde{\*x}_n^T \bm w))^{1 - y_n}
	\end{myalign*}
	\item The log-likelihood:
	\begin{myalign*}
	    \L_{MLE}(\bm w) = \sum_{n = 1}^N
	    \left(
	    	y_n \tilde{\*x}_n^T \bm w - \log(1 + \exp(\tilde{\*x}_n^T \bm w))
	    \right)
	\end{myalign*}
	...
	\begin{myalign*}
	    ...
	    \left( 
	    	\tilde{\*x}_n y_n - \tilde{\*x}_n \sigma(\tilde{\*x}_n^T \bm w)
	    	\right) \\
	    &= - \*X^T[\sigma(\*X \bm w) - \*y]
	\end{myalign*}
	...
	\begin{itemize}
		...
		\begin{myalign*}
		    \*H(\bm w) &= - \frac{d \*g(\bm w)}{d \bm w^T}  = \sum_{n = 1}^N \frac{d}{d \bm w^T} \sigma(\tilde{\*x}_n^T \bm w) \tilde{\*x}_n\\
		    &= \sum_{n = 1}^N \tilde{\*x}_n \sigma(\tilde{\*x}_n^T \bm w)(1 - \sigma(\tilde{\*x}_n^T \bm w)) \tilde{\*x}_n^T\\
		    &= \tilde{\*X}^T \*S \tilde{\*X}
		\end{myalign*}
		where $\*S$ is a $N \times N$ diagonal matrix with diagonals
		\begin{myalign*}
		    S_{nn} = \sigma(\tilde{\*x}_n^T \bm w)(1 - \sigma(\tilde{\*x}_n^T \bm w))
		\end{myalign*}
...
\begin{itemize}
	...
	\begin{myalign*}
	    \min_{\bm w} \L(\bm w) = - \sum_{n = 1}^N \log(p(y_n | \tilde{\*x}^T_n \bm w))
	\end{myalign*}
	where $p(y_n | \tilde{\*x}^T_n \bm w)$ is an exponential family distribution
	\item We obtain the solution
	\begin{myalign*}
	    \frac{d \L}{d \bm w} = \tilde{\*X}^T[\*g^{-1}(\bm \eta) - \bm \phi(\*y)]
	\end{myalign*}
\end{itemize}
...
\begin{itemize}
	...
	\item Thus we have
	\begin{myalign*}
	    \bm w = \sum_{n = 1}^N \alpha_n \*x_n, \hspace{5pt} \*y = \sum_{d = 1}^D w_d \*{\bar{x}}_d
	\end{myalign*}
...
\begin{itemize}
	...
	\begin{myalign*}
	    g(\bm w) = \min_{\bm w} \sum_{n = 1}^N [1 - y_n \tilde{\bm \phi}_n^T \bm w]_+ + \frac{\lambda}{2} \sum_{j = 1}^M w^2_j
	\end{myalign*}
	...
	\begin{itemize}
		...
		\begin{myalign*}
		    \min_{\bm w} \max_{\bm \alpha \in [0, C]^N} \sum_{n = 1}^N \alpha_n (1 - y_n \tilde{\bm \phi}_n^T \bm w) + \frac{1}{2} \sum_{j = 1}^M w_j^2
		\end{myalign*}
		...
		\item Equating this to 0, we get:
			\begin{myalign*}
			    &\bm w^*_{1 : M} = \sum^N_{n = 1} \alpha_n y_n \phi_n = \bm \Phi^T \text{diag}(\*y) \bm \alpha\\
			    &\bm \alpha^T \*y = 0
			\end{myalign*}
...
\begin{itemize}
    ...
    \end{align*}
    \item Reduce complexity using bayesian rule. %LEARN
...
\begin{itemize}
    ...
    \begin{myalign*}
        \hat{y}_n = g((\bm w^{(K - 1)})^T * h(\*B^{(K - 2)} * h(* ... * h(\*B^{(1)} * \*x_n)))
    \end{myalign*}
    ...
    \begin{enumerate}
    	...
    	\begin{myalign*}
    	    \frac{d \bm \L}{d\*B^{(k)}} = \sum_n \bm \delta_n^{(k)} (\*z_n^{(k)})^T
    	\end{myalign*}
    \end{enumerate}
\end{itemize}
% ========================================================
